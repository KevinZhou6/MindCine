...
class myTransformer(nn.Module):
    ...
    def __init__(self, d_model=2048):
        ...
        self.proj_frames = nn.Conv2d(7,17,kernel_size=1)
...
chosed_label = [1, 10, 12, 16, 19, 23, 31, 34, 37, 39]
...
if __name__ == "__main__":
    ...
    latent_data = np.load('data/Perception/1200_latent.npy')
    latent_data = torch.from_numpy(latent_data)
    test_latent = torch.load('data/Perception/10classes_latents.pt')
    # test_latent = torch.from_numpy(test_latent)
    # print(latent_data)

    print(eegdata.shape,latent_data.shape,test_latent.shape)
    
    latent_data = latent_data.to(torch.float16)
    #test_latent = test_latent.to(torch.float16)
    chosed_index = []
    for i in range(7):
        index = [list(GT_label[i]).index(element) for element in chosed_label]
        chosed_index.append(index)
    # print(chosed_index)
    new_eeg = np.zeros((7, 10, 5, 62, 400))
    for i in range(7):
    ...
    # print(latent_data.shape)
    new_latent = np.zeros((6, 10, 5, 4, 6, 36, 64))
    for i in range(6):
        new_latent[i] = latent_data[i][chosed_index[i], :, :, :, :, :]
    # test_latent = new_latent[6:7]
    # new_latent = new_latent[:6]
    new_latent = rearrange(new_latent, "g p d c f h w -> (g p d) c f h w")
    # test_latent = rearrange(test_latent, "p d c f h w -> (p d) c f h w")
    ...
    latent_data = rearrange(new_latent, "b c f h w -> b f c h w")
    test_latent = rearrange(test_latent, "b c f h w -> b f c h w")
    # EEG = EEG[:250,...]
    # latent_data = latent_data[:250,...]
    print("EEG:", EEG.shape)  
    print('test_eeg',test_eeg.shape)
    print("latent:", latent_data.shape) 
    ...
    test_dataset = Dataset(test_eeg,test_latent)
    ...
    for x,y in test_loader:
        ...
        full_video = torch.cat((padded_video, y), dim=1).float().cuda()
        txt_label, out = model(x, full_video)
        latent_out = out[:, :-1, :].cpu().detach().numpy()
        
        if latents is None:
            latents = latent_out
        else:
            latents = np.concatenate((latents, latent_out), axis=0)
    # latent_out = np.array(latent_out)
    print(latents.shape)
    np.save('latents/mindcine_10_classes.npy', latents)
