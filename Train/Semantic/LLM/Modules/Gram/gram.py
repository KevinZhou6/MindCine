...
import sys 
sys.path.append('/userhome2/zhoutianyi/BrainDecoding/MindCine/Train/Semantic/LLM/Modules/Gram')
...
from transformer import Block, trunc_normal_, LabelSmoothingCrossEntropy, TConv
from helper import get_1d_sincos_pos_embed_from_grid
from modeling_vqkd import VQKD
...
class Gram(nn.Module):
    ...
    def load_vqgan(cf):
        # print(cf.gpu)
        checkpoint = torch.load(cf.vqgan_model_path, map_location='cpu',weights_only=False)
    ...
    def forward(self, x, ch_list):
        ...
        latent= self.forward_encoder(x, ch_list)
...
if __name__=='__main__':
    ...
    checkpoint = torch.load('base.pth', map_location='cpu',weights_only=False)  
    pre_cf = checkpoint['cf']
    pre_cf.if_finetune = cf.if_finetune
    pre_cf.if_scratch = cf.if_scratch
    pre_cf.n_class = cf.n_class   
    pre_cf.vqgan_model_path = 'base_class_quantization.pth'  #optinal. If you need to use the pre-trained stage2 model, remember to add this command and change the cf.vqgan_model_path to the path of base_class_quantization.pth
    ...
    data = torch.rand(1,62,200).to(device=cf.device)
