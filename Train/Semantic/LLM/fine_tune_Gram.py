...
from Modules.CBraMod.cbramod import CBraMod
import timm.models
from timm.models import create_model
import torch
from loss import ClipLoss,soft_clip_loss,GroupLoss
import torch.nn.functional as F
ch_list =  ['Fp1', 'Fpz', 'Fp2', 'AF3', 'AF4', 'F7', 'F5', 'F3', 'F1', 'Fz', 'F2', 'F4',
            ...
            'CB1', 'O1', 'Oz', 'O2', 'CB2']
from Modules.Gram.gram import Gram
class LitEEGPTCausal(pl.LightningModule):
    
    def __init__(self):
        super().__init__() 
        
        from Modules.Gram.config import Config, get_param_sets
        config = Config('Modules/Gram/finetune.yaml')
        ...
        checkpoint = torch.load('Modules/Gram/base.pth', map_location='cpu',weights_only=False)  
        pre_cf = checkpoint['cf']
        pre_cf.if_finetune = cf.if_finetune
        pre_cf.if_scratch = cf.if_scratch
        pre_cf.n_class = cf.n_class   
        pre_cf.vqgan_model_path = 'Modules/Gram/base_class_quantization.pth'  #optinal. If you need to use the pre-trained stage2 model, remember to add this command and change the cf.vqgan_model_path to the path of base_class_quantization.pth
...
# chosed_label = [1, 10, 12, 16, 19, 24, 25, 31, 34, 39]
...
image_768 = torch.load('/data/SEED-DV/image_emb_768.pt',map_location='cpu')
text_768 = torch.load('data/SEED-DV/text_emb_768.pt',map_location='cpu')
depth = torch.load('data/SEED-DV/depth_768.pt',map_location='cpu')
...
checkpoint_callback = ModelCheckpoint(
    dirpath=f'../checkpoints/Gram/{current_time}/',  # 模型保存的文件夹
    filename='best_model_10_classes-sub1',  # 文件名
...
trainer = pl.Trainer(accelerator='cuda',
                        ...
                        callbacks=callbacks,
                        logger=[ 
                                pl_loggers.CSVLogger('../logs/', name="Gram_csv",version=f'{current_time}')],
