...
LastEditTime: 2025-04-19 13:05:43
LastEditors:  
'''
import numpy as np
...
class clip_score:
    ...
    def __init__(self, 
        ...
        self.device = device
        self.clip_model = CLIPVisionModelWithProjection.from_pretrained('/userhome/zhoutianyi/Zhoutianyi/Mutilmodel/huggingface/clip-vit-large-patch14').to(device, torch.float16)
        self.clip_processor = AutoProcessor.from_pretrained('/userhome/zhoutianyi/Zhoutianyi/Mutilmodel/huggingface/clip-vit-large-patch14')
...
def img_classify_metric(
    ...
    assert n_way > top_k
    processor = ViTImageProcessor.from_pretrained('/userhome2/zhoutianyi/huggingface/vit-base-patch16-224')
    model = ViTForImageClassification.from_pretrained('/userhome2/zhoutianyi/huggingface/vit-base-patch16-224').to(device, torch.float16)
    model.eval()
...
def video_classify_metric(
    ...
    assert n_way > top_k
    processor = VideoMAEImageProcessor.from_pretrained('/userhome2/zhoutianyi/huggingface/videomae-base-short-finetuned-kinetics')
    model = VideoMAEForVideoClassification.from_pretrained('/userhome2/zhoutianyi/huggingface/videomae-base-short-finetuned-kinetics', num_frames=num_frames).to(device, torch.float16)
    model.eval()
...
def hue_score_only(
    ...
    scores = []
    for pred, gt in zip(pred_videos, gt_videos):
        scores.append(calculate_hue_similarity(pred, gt))
    return np.mean(scores), np.std(scores)
...
chosed_label = [28,14,23,5,19,34,10,37,12,17,28,14,23,5,19,34,10,37,12,17]
...
image_psnr = []
image_hue = []
gt_path ='/userhome2/zhoutianyi/Dataset/SEED-DV/video_16_9_512/block7/'
pred_path ='/userhome2/zhoutianyi/BrainDecoding/MindCine/Generation/Tune-A-Video/10classes_RenJI/normal/fusuying/'
# seed_everything(42)
for i in range(80):
    ...
    mean, std = ssim_score_only(pred_video,gt_video)
    image_ssim.append(mean)

    mean,std = psnr_score_only(pred_video,gt_video)
    image_psnr.append(mean)

    mean,std = hue_score_only(pred_video,gt_video)
    image_hue.append(mean)
...
print('### 10_Classes_Fullmodel/CineMind #')
...
print("image_psnr = ", np.mean(np.array(image_psnr)), np.std(np.array(image_psnr)))
print("image_hue = ", np.mean(np.array(image_hue)), np.std(np.array(image_hue)))
