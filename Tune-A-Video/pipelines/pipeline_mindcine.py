...
LastEditTime: 2025-04-11 02:40:26
LastEditors:  
'''
...
class TuneAVideoPipeline(DiffusionPipeline):
    ...
    def _encode_eeg(self, model, eeg, device, num_videos_per_eeg, do_classifier_guidance, negative_eeg):
        ...
        #     eeg_embeddings[0, i] = eeg_embeddings[0, 76]
        ...
        if do_classifier_guidance:
            # uncond_input = negative_eeg.to(device)
            # uncond_embeddings = model(uncond_input)
            # uncond_embeddings = torch.reshape(uncond_embeddings, [uncond_embeddings.shape[0], 77, 768]).half()
            ...
            #uncond_embeddings = np.load('/home/v-xuanhaoliu/EEG2Video/Tune-A-Video/negative.npy')
            if negative_eeg is not None:
                negative = negative_eeg
                uncond_embeddings = negative.unsqueeze(0)
            else:
                uncond_embeddings = torch.load('Tune-A-Video/negative.pt',map_location='cpu')
            # print(uncond_embeddings.shape)
            # uncond_embeddings = uncond_embeddings.unsqueeze(0)
            uncond_embeddings = uncond_embeddings.cuda().to(dtype=torch.float16)
            #uncond_embeddings = negative.unsqueeze(0).cuda().to(dtype=torch.float16)
    ...
    def __call__(
        ...
        negative_eeg: Optional[Union[str, List[str]]] = None,
        num_videos_per_eeg: Optional[int] = 1,
        eta: float = 0.0,
